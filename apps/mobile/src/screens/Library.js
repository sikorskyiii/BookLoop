    ...
    <View style={{ flex:1, backgroundColor: theme.colors.bg }}>
      ...
      <ScrollView horizontal showsHorizontalScrollIndicator={false} style={{ paddingHorizontal: 16, paddingTop: 12 }}>
        {filtered.slice(0,5).map((item) => (
          <View key={item.id || item.title} style={{
            width:220, height:140, marginRight:12, borderRadius:16, overflow:"hidden",
            borderWidth:1, borderColor: theme.colors.border, backgroundColor: theme.colors.card
          }}>
            <Text style={{ color: theme.colors.text, fontWeight:"700", fontSize:14, padding:12 }} numberOfLines={2}>
              {item.title}
            </Text>
          </View>
        ))}
      ...
      <FlatList
        ...
        ListEmptyComponent={<Text style={{ color: theme.colors.textMuted, textAlign:"center", marginTop: 32 }}>Порожньо</Text>}
      />
